...
flowchart TD
    ...
    subgraph Pool3["企业IPR"]
        Pool3_T1("执行业务操作")
    end
    Start --> Pool1_T1
    Pool1_Done -.->|交付预审案件| Pool2_T1
...
|------|------|----------|----------|---------|--------|
| 代理师 | 浏览预审案件 | 登录后进入工作台 | 预审案件列表页(precheck-case-list) | 预审案件 | 业务数据 |
| 预审员 | 执行业务操作 | 登录后进入工作台 | - | 预审案件 | 业务数据 |
| 企业IPR | 执行业务操作 | 登录后进入工作台 | - | - | 业务数据 |
...
```mermaid
flowchart LR
    DS1[["预审案件<br/>字段: case_no, applicant, tech_field, reviewer..."]]
    P1("新增案件<br/>(预审案件)")
    P2("查看案件<br/>(预审案件)")
    P3("开始预审<br/>(预审案件)")
    P4("通过<br/>(预审案件)")
    ...
    P3 -.->|查询结果| Ext2
    P3 -.->|查询结果| Ext3
    Ext1 -->|confirm 预审案件| P4
    Ext2 -->|confirm 预审案件| P4
    P4 -.->|CRUD| DS1
    P4 -.->|查询结果| Ext1
    P4 -.->|查询结果| Ext2
    P4 -.->|查询结果| Ext3
...
|------|------------|----------|----------|----------|---------|---------|
| 预审案件 | 代理师 | 代理师/预审员/企业IPR | 代理师/预审员 | 代理师 → [预审案件] → 代理师 | id | case_no, applicant, tech_field |
...
flowchart TD
    Start((用户登录))
    P01["预审案件列表页(precheck-case-list)<br/>/precheck-cases"]
    P02["预审案件详情页(precheck-case-detail)<br/>/precheck-cases/:id"]
    P03["预审案件表单页(precheck-case-submit)<br/>/precheck-cases/form"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | 预审案件列表页(precheck-case-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| 预审案件列表页(precheck-case-list) | 预审案件详情页(precheck-case-detail) | 点击新增按钮 | precheck-case | 已认证 | 路由跳转 |
| 预审案件详情页(precheck-case-detail) | 预审案件表单页(precheck-case-submit) | 点击"开始预审"按钮 | precheck-case | 已认证 | 路由跳转 |
| 预审案件表单页(precheck-case-submit) | 预审案件列表页(precheck-case-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:57:00.854Z）
