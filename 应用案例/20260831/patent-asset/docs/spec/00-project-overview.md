...
基于需求模型中的 5 个业务实体，系统划分为以下模块：

| 模块 | 核心实体 | 职责 |
|------|---------|------|
| PatentAsset管理 | 专利资产 | 专利资产数据的增删改查、状态流转、权限控制 |
| ValuationRecord管理 | 估值记录 | 估值记录数据的增删改查、状态流转、权限控制 |
| AnnualFeeRecord管理 | 年费记录 | 年费记录数据的增删改查、状态流转、权限控制 |
| PledgeTransferRecord管理 | 质押转让记录 | 质押转让记录数据的增删改查、状态流转、权限控制 |
| MatchDeal管理 | 撮合意向 | 撮合意向数据的增删改查、状态流转、权限控制 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-08-28T16:41:57.840Z）
