...
### 3.1 executive-dashboard (P01)

```mermaid
flowchart TB
    P[P01 executive-dashboard]
...
### 3.3 valuation-center (P03)

```mermaid
flowchart TB
    P[P03 valuation-center]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
...
### 3.4 portfolio-strategy (P04)

```mermaid
flowchart TB
    P[P04 portfolio-strategy]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
flowchart TB
    ...
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[撮合意向Form 新增/编辑表单]
    P --> Detail[撮合意向Detail 详情弹窗]
```

### 3.6 pledge-transfer (P06)

```mermaid
flowchart TB
    P[P06 pledge-transfer]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[质押转让记录Form 新增/编辑表单]
    P --> Detail[质押转让记录Detail 详情弹窗]
```
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /dashboard | executive-dashboard页面主组件 |
| P02Page | /patents | patent-ledger页面主组件 |
| P03Page | /valuations | valuation-center页面主组件 |
| P04Page | /portfolio | portfolio-strategy页面主组件 |
| P05Page | /matches | match-channel页面主组件 |
| P06Page | /pledges | pledge-transfer页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-08-28T16:41:57.841Z）
