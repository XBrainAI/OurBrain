...
本文档基于需求模型中的 6 个页面，推导前端组件架构与布局方案。
...
### 3.1 monitor-dashboard (P01)

```mermaid
flowchart TB
    P[P01 monitor-dashboard]
...
### 3.2 infringement-leads (P02)

```mermaid
flowchart TB
    P[P02 infringement-leads]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[侵权线索Form 新增/编辑表单]
    P --> Detail[侵权线索Detail 详情弹窗]
```

### 3.3 evidence-detail (P03)

```mermaid
flowchart TB
    P[P03 evidence-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
flowchart TB
    ...
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[维权案件Form 新增/编辑表单]
    P --> Detail[维权案件Detail 详情弹窗]
```

### 3.5 cross-border-rules (P05)

```mermaid
flowchart TB
    P[P05 cross-border-rules]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[跨境规则模板Form 新增/编辑表单]
    P --> Detail[跨境规则模板Detail 详情弹窗]
```

### 3.6 rights-analytics (P06)

```mermaid
flowchart TB
    P[P06 rights-analytics]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Cards[StatisticCards 指标卡片]
    P --> Charts[Charts 图表区]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /dashboard | monitor-dashboard页面主组件 |
| P02Page | /leads | infringement-leads页面主组件 |
| P03Page | /evidence | evidence-detail页面主组件 |
| P04Page | /cases | rights-cases页面主组件 |
| P05Page | /rules | cross-border-rules页面主组件 |
| P06Page | /analytics | rights-analytics页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-08-29T15:52:07.042Z）
