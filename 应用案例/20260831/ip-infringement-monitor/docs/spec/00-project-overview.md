...
基于需求模型中的 4 个业务实体，系统划分为以下模块：

| 模块 | 核心实体 | 职责 |
|------|---------|------|
| InfringementLead管理 | 侵权线索 | 侵权线索数据的增删改查、状态流转、权限控制 |
| EvidenceRecord管理 | 存证记录 | 存证记录数据的增删改查、状态流转、权限控制 |
| RightsCase管理 | 维权案件 | 维权案件数据的增删改查、状态流转、权限控制 |
| CrossBorderRule管理 | 跨境规则模板 | 跨境规则模板数据的增删改查、状态流转、权限控制 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-08-29T15:52:07.036Z）
