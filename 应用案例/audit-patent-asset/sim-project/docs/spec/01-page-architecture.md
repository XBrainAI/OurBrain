...
本文档基于需求模型中的 4 个页面，推导前端组件架构与布局方案。
...
### 3.1 patent-asset-list (P01)

```mermaid
flowchart TB
    P[P01 patent-asset-list]
...
### 3.2 patent-asset-detail (P02)

```mermaid
flowchart TB
    P[P02 patent-asset-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 patent-asset-evaluate (P03)

```mermaid
flowchart TB
    P[P03 patent-asset-evaluate]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Form[StepForm 分步表单]
```

### 3.4 asset-dashboard (P04)

```mermaid
flowchart TB
    P[P04 asset-dashboard]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /patent-assets | patent-asset-list页面主组件 |
| P02Page | /patent-assets/:id | patent-asset-detail页面主组件 |
| P03Page | /patent-assets/:id/evaluate | patent-asset-evaluate页面主组件 |
| P04Page | /patent-assets/dashboard | asset-dashboard页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:12:23.234Z）
