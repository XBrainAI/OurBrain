...
### 3.1 green-patent-list (P01)

```mermaid
flowchart TB
    P[P01 green-patent-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[绿色专利Form 新增/编辑表单]
    P --> Detail[绿色专利Detail 详情弹窗]
```

### 3.2 green-patent-detail (P02)

```mermaid
flowchart TB
    P[P02 green-patent-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 green-patent-layout (P03)

```mermaid
flowchart TB
    P[P03 green-patent-layout]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /green-patents | green-patent-list页面主组件 |
| P02Page | /green-patents/:id | green-patent-detail页面主组件 |
| P03Page | /green-patents/form | green-patent-layout页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:57:34.135Z）
