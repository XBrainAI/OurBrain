...
flowchart TD
    ...
    subgraph Pool2["绿色技术研究员"]
        Pool2_T1("执行业务操作")
    end
    subgraph Pool3["布局规划师"]
        Pool3_T1("执行业务操作")
    end
    Start --> Pool1_T1
    Pool1_Done -.->|交付绿色专利| Pool2_T1
    Pool2_Done -.->|交付绿色专利| Pool3_T1
    Pool3_Done --> End((业务完成))
```
...
|------|------|----------|----------|---------|--------|
| 企业IPR | 浏览绿色专利 | 登录后进入工作台 | 绿色专利列表页(green-patent-list) | 绿色专利 | 业务数据 |
| 绿色技术研究员 | 执行业务操作 | 登录后进入工作台 | - | 绿色专利 | 业务数据 |
| 布局规划师 | 执行业务操作 | 登录后进入工作台 | - | 绿色专利 | 业务数据 |
...
```mermaid
flowchart LR
    System((本系统<br/>绿色低碳新质产业专项专利布局分析平台))
    Ext1["企业IPR"]
    Ext2["绿色技术研究员"]
    Ext3["布局规划师"]
    Ext1 -->|绿色专利数据| System
...
```mermaid
flowchart LR
    DS1[["绿色专利<br/>字段: patent_no, title, green_field, analyst..."]]
    P1("新增专利<br/>(绿色专利)")
    P2("查看专利<br/>(绿色专利)")
    P3("生成布局方案<br/>(绿色专利)")
    P4("审核布局<br/>(绿色专利)")
    ...
    P3 -.->|查询结果| Ext2
    P3 -.->|查询结果| Ext3
    Ext1 -->|confirm 绿色专利| P4
    Ext2 -->|confirm 绿色专利| P4
    Ext3 -->|confirm 绿色专利| P4
    P4 -.->|CRUD| DS1
    P4 -.->|查询结果| Ext1
    P4 -.->|查询结果| Ext2
    P4 -.->|查询结果| Ext3
...
|------|------------|----------|----------|----------|---------|---------|
| 绿色专利 | 企业IPR | 企业IPR/绿色技术研究员/布局规划师 | 企业IPR/绿色技术研究员/布局规划师 | 企业IPR → [绿色专利] → 企业IPR | id | patent_no, title, green_field |
...
flowchart TD
    Start((用户登录))
    P01["绿色专利列表页(green-patent-list)<br/>/green-patents"]
    P02["绿色专利详情页(green-patent-detail)<br/>/green-patents/:id"]
    P03["绿色专利表单页(green-patent-layout)<br/>/green-patents/form"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | 绿色专利列表页(green-patent-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| 绿色专利列表页(green-patent-list) | 绿色专利详情页(green-patent-detail) | 点击新增按钮 | green-patent | 已认证 | 路由跳转 |
| 绿色专利详情页(green-patent-detail) | 绿色专利表单页(green-patent-layout) | 点击"生成布局方案"按钮 | green-patent | 已认证 | 路由跳转 |
| 绿色专利表单页(green-patent-layout) | 绿色专利列表页(green-patent-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:57:34.099Z）
