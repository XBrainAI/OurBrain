...
### 3.1 pledge-case-list (P01)

```mermaid
flowchart TB
    P[P01 pledge-case-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[质押案件Form 新增/编辑表单]
    P --> Detail[质押案件Detail 详情弹窗]
```

### 3.2 pledge-case-detail (P02)

```mermaid
flowchart TB
    P[P02 pledge-case-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 pledge-case-apply (P03)

```mermaid
flowchart TB
    P[P03 pledge-case-apply]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /pledge-cases | pledge-case-list页面主组件 |
| P02Page | /pledge-cases/:id | pledge-case-detail页面主组件 |
| P03Page | /pledge-cases/form | pledge-case-apply页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:18:59.234Z）
