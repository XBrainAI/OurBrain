...
### 3.1 fto-analysis-list (P01)

```mermaid
flowchart TB
    P[P01 fto-analysis-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[FTO分析Form 新增/编辑表单]
    P --> Detail[FTO分析Detail 详情弹窗]
```

### 3.2 fto-analysis-detail (P02)

```mermaid
flowchart TB
    P[P02 fto-analysis-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 fto-analysis-form (P03)

```mermaid
flowchart TB
    P[P03 fto-analysis-form]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /fto-analyses | fto-analysis-list页面主组件 |
| P02Page | /fto-analyses/:id | fto-analysis-detail页面主组件 |
| P03Page | /fto-analyses/form | fto-analysis-form页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:56:45.721Z）
