...
|------|------|----------|----------|---------|--------|
| 企业IPR | 浏览FTO分析 | 登录后进入工作台 | FTO分析列表页(fto-analysis-list) | FTO分析 | 业务数据 |
| FTO分析师 | 执行业务操作 | 登录后进入工作台 | - | FTO分析 | 业务数据 |
| 外贸经理 | 执行业务操作 | 登录后进入工作台 | - | - | 业务数据 |
...
```mermaid
flowchart LR
    System((本系统<br/>全球多区域FTO智能合规风控系统))
    Ext1["企业IPR"]
    Ext2["FTO分析师"]
    Ext3["外贸经理"]
    Ext1 -->|FTO分析数据| System
...
```mermaid
flowchart LR
    DS1[["FTO分析<br/>字段: product_name, target_market, risk_level, analyst..."]]
    P1("新建FTO分析<br/>(FTO分析)")
    P2("查看分析<br/>(FTO分析)")
    P3("开始分析<br/>(FTO分析)")
    P4("提交报告<br/>(FTO分析)")
    ...
    Ext2["FTO分析师"]
    Ext3["外贸经理"]
    Ext1 -->|create FTO分析| P1
    Ext2 -->|create FTO分析| P1
    P1 -->|写入| DS1
    P1 -.->|查询结果| Ext1
    P1 -.->|查询结果| Ext2
    P1 -.->|查询结果| Ext3
    ...
    P2 -.->|查询结果| Ext2
    P2 -.->|查询结果| Ext3
    Ext1 -->|update FTO分析| P3
    Ext2 -->|update FTO分析| P3
    P3 -->|写入| DS1
    P3 -.->|查询结果| Ext1
    P3 -.->|查询结果| Ext2
    P3 -.->|查询结果| Ext3
    Ext1 -->|finish FTO分析| P4
    Ext2 -->|finish FTO分析| P4
    P4 -.->|CRUD| DS1
    P4 -.->|查询结果| Ext1
    P4 -.->|查询结果| Ext2
    P4 -.->|查询结果| Ext3
    ...
    P5 -.->|查询结果| Ext2
    P5 -.->|查询结果| Ext3
    Ext1 -->|cancel FTO分析| P6
    Ext2 -->|cancel FTO分析| P6
    P6 -.->|CRUD| DS1
    P6 -.->|查询结果| Ext1
    P6 -.->|查询结果| Ext2
    P6 -.->|查询结果| Ext3
...
|------|------------|----------|----------|----------|---------|---------|
| FTO分析 | 企业IPR | 企业IPR/FTO分析师/外贸经理 | 企业IPR/FTO分析师 | 企业IPR → [FTO分析] → 企业IPR | id | product_name, target_market, risk_level |
...
flowchart TD
    Start((用户登录))
    P01["FTO分析列表页(fto-analysis-list)<br/>/fto-analyses"]
    P02["FTO分析详情页(fto-analysis-detail)<br/>/fto-analyses/:id"]
    P03["FTO分析表单页(fto-analysis-form)<br/>/fto-analyses/form"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | FTO分析列表页(fto-analysis-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| FTO分析列表页(fto-analysis-list) | FTO分析详情页(fto-analysis-detail) | 点击新建按钮 | fto-analysis | 已认证 | 路由跳转 |
| FTO分析详情页(fto-analysis-detail) | FTO分析表单页(fto-analysis-form) | 点击"开始分析"按钮 | fto-analysis | 已认证 | 路由跳转 |
| FTO分析表单页(fto-analysis-form) | FTO分析列表页(fto-analysis-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:56:45.687Z）
