# AGENTS.md — Demora 项目运行时指引

> 本文件由 `scaffold-project.mjs` 自动注入。如已有 AGENTS.md，原文件已备份为 `AGENTS.md.bak`。

## 一、项目说明

本项目由 Demora Skill 管理，遵循三阶段渐进式研发流程。
...
1. 读取 `skills/frontend-design/SKILL.md` 后再设计 HTML（方法论不经翻译/适配/字段化中转）
2. HTML 必须包含 4 项最小契约：
...
- 代码注释使用中文，变量/函数名使用英文
- 代码文件禁止使用中文文件名
- Windows 路径使用原始字符串 `r""` 或 `pathlib.Path`
