...
### 3.1 oa-response-list (P01)

```mermaid
flowchart TB
    P[P01 oa-response-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[OA答复Form 新增/编辑表单]
    P --> Detail[OA答复Detail 详情弹窗]
```

### 3.2 oa-response-detail (P02)

```mermaid
flowchart TB
    P[P02 oa-response-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 oa-response-form (P03)

```mermaid
flowchart TB
    P[P03 oa-response-form]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /oa-responses | oa-response-list页面主组件 |
| P02Page | /oa-responses/:id | oa-response-detail页面主组件 |
| P03Page | /oa-responses/form | oa-response-form页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:08:57.993Z）
