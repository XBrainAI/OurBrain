...
- **项目定位**：OA 答复拆解、案例匹配、修改推演与标准化输出平台
- **核心差异点**：整合 1 大业务实体（OA答复），提供全流程数字化管理
- **关键功能**：浏览oa-response、查看oa-response详情、新增/编辑oa-response
...
| 字段 | 内容 |
|------|------|
| 项目名称 | AI专利审查意见OA智能答复辅助专家系统 |
| 产品愿景 | OA 答复拆解、案例匹配、修改推演与标准化输出平台 |
| 所属领域 | PATENT |
...
| 维度 | 内容 |
|------|------|
| 产品名称 | AI专利审查意见OA智能答复辅助专家系统 |
| 产品描述 | OA 答复拆解、案例匹配、修改推演与标准化输出平台 |
| 所属领域 | PATENT |
| 实体数量 | 1 个 |
| 角色数量 | 3 个 |
| 页面数量 | 3 个 |
...
| ID | 页面名称 | 类型 | 关联实体 | 路由 |
|------|---------|------|---------|------|
| P01 | oa-response-list | CRUD 列表页 | oa-response | /oa-responses |
| P02 | oa-response-detail | 详情页 | oa-response | /oa-responses/:id |
| P03 | oa-response-form | 表单页 | oa-response | /oa-responses/form |


---

> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:08:57.947Z）
