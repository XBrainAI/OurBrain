...
|------|------|----------|----------|---------|--------|
| 代理师 | 浏览OA答复 | 登录后进入工作台 | OA答复列表页(oa-response-list) | OA答复 | 业务数据 |
| 企业IPR | 执行业务操作 | 登录后进入工作台 | - | OA答复 | 业务数据 |
| 审查员 | 执行业务操作 | 登录后进入工作台 | - | - | 业务数据 |
...
```mermaid
flowchart LR
    System((本系统<br/>AI专利审查意见OA智能答复辅助专家系统))
    Ext1["代理师"]
    Ext2["企业IPR"]
    Ext3["审查员"]
    Ext1 -->|OA答复数据| System
...
```mermaid
flowchart LR
    DS1[["OA答复<br/>字段: case_no, application_no, rejection_type, agent..."]]
    P1("新建答复<br/>(OA答复)")
    P2("查看答复<br/>(OA答复)")
    P3("开始拆解<br/>(OA答复)")
    P4("生成草稿<br/>(OA答复)")
...
|------|------------|----------|----------|----------|---------|---------|
| OA答复 | 代理师 | 代理师/企业IPR/审查员 | 代理师/企业IPR | 代理师 → [OA答复] → 代理师 | id | case_no, application_no, rejection_type |
...
flowchart TD
    Start((用户登录))
    P01["OA答复列表页(oa-response-list)<br/>/oa-responses"]
    P02["OA答复详情页(oa-response-detail)<br/>/oa-responses/:id"]
    P03["OA答复表单页(oa-response-form)<br/>/oa-responses/form"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | OA答复列表页(oa-response-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| OA答复列表页(oa-response-list) | OA答复详情页(oa-response-detail) | 点击"新建答复"按钮 | oa-response | 已认证 | 路由跳转 |
| OA答复详情页(oa-response-detail) | OA答复表单页(oa-response-form) | 点击"开始拆解"按钮 | oa-response | 已认证 | 路由跳转 |
| OA答复表单页(oa-response-form) | OA答复列表页(oa-response-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
- **OA答复**：已收到OA / 拆解中 / 草拟中 / 已答复 / 已授权 / 最终驳回
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:08:57.968Z）
