...
flowchart TD
    ...
    Start --> Pool1_T1
    Pool1_Done -.->|交付AIGC专利| Pool2_T1
    Pool2_Done -.->|交付AIGC专利| Pool3_T1
    Pool3_Done --> End((业务完成))
```
...
|------|------|----------|----------|---------|--------|
| AI工程师 | 浏览AIGC专利 | 登录后进入工作台 | AIGC专利列表页(aigc-patent-list) | AIGC专利 | 业务数据 |
| 企业IPR | 执行业务操作 | 登录后进入工作台 | - | AIGC专利 | 业务数据 |
| 代理师 | 执行业务操作 | 登录后进入工作台 | - | AIGC专利 | 业务数据 |
...
```mermaid
flowchart LR
    System((本系统<br/>AIGC软件算法类数字专利专项管理平台))
    Ext1["AI工程师"]
    Ext2["企业IPR"]
    Ext3["代理师"]
    Ext1 -->|AIGC专利数据| System
    System -.->|查询结果/反馈| Ext1
    Ext2 -->|AIGC专利数据| System
    System -.->|查询结果/反馈| Ext2
    Ext3 -->|AIGC专利数据| System
    System -.->|查询结果/反馈| Ext3
```
...
```mermaid
flowchart LR
    DS1[["AIGC专利<br/>字段: patent_no, algorithm_type, code_artifact, ownership_type..."]]
    P1("新增专利<br/>(AIGC专利)")
    P2("查看专利<br/>(AIGC专利)")
    P3("触发分析<br/>(AIGC专利)")
    P4("审核权属<br/>(AIGC专利)")
    ...
    Ext2["企业IPR"]
    Ext3["代理师"]
    Ext1 -->|create AIGC专利| P1
    Ext2 -->|create AIGC专利| P1
    Ext3 -->|create AIGC专利| P1
    P1 -->|写入| DS1
    P1 -.->|查询结果| Ext1
    P1 -.->|查询结果| Ext2
    P1 -.->|查询结果| Ext3
    ...
    P2 -.->|查询结果| Ext2
    P2 -.->|查询结果| Ext3
    Ext1 -->|update AIGC专利| P3
    Ext2 -->|update AIGC专利| P3
    Ext3 -->|update AIGC专利| P3
    P3 -->|写入| DS1
    P3 -.->|查询结果| Ext1
    P3 -.->|查询结果| Ext2
    P3 -.->|查询结果| Ext3
    Ext1 -->|confirm AIGC专利| P4
    Ext2 -->|confirm AIGC专利| P4
    Ext3 -->|confirm AIGC专利| P4
    P4 -.->|CRUD| DS1
    P4 -.->|查询结果| Ext1
    P4 -.->|查询结果| Ext2
    P4 -.->|查询结果| Ext3
    ...
    P5 -.->|查询结果| Ext2
    P5 -.->|查询结果| Ext3
    Ext1 -->|cancel AIGC专利| P6
    Ext2 -->|cancel AIGC专利| P6
    Ext3 -->|cancel AIGC专利| P6
    P6 -.->|CRUD| DS1
    P6 -.->|查询结果| Ext1
    P6 -.->|查询结果| Ext2
    P6 -.->|查询结果| Ext3
...
|------|------------|----------|----------|----------|---------|---------|
| AIGC专利 | AI工程师 | AI工程师/企业IPR/代理师 | AI工程师/企业IPR/代理师 | AI工程师 → [AIGC专利] → AI工程师 | id | patent_no, algorithm_type, code_artifact |
...
flowchart TD
    Start((用户登录))
    P01["AIGC专利列表页(aigc-patent-list)<br/>/aigc-patents"]
    P02["AIGC专利详情页(aigc-patent-detail)<br/>/aigc-patents/:id"]
    P03["AIGC专利表单页(aigc-patent-upload)<br/>/aigc-patents/form"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | AIGC专利列表页(aigc-patent-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| AIGC专利列表页(aigc-patent-list) | AIGC专利详情页(aigc-patent-detail) | 点击新增按钮 | aigc-patent | 已认证 | 路由跳转 |
| AIGC专利详情页(aigc-patent-detail) | AIGC专利表单页(aigc-patent-upload) | 点击"触发分析"按钮 | aigc-patent | 已认证 | 路由跳转 |
| AIGC专利表单页(aigc-patent-upload) | AIGC专利列表页(aigc-patent-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:20:38.236Z）
