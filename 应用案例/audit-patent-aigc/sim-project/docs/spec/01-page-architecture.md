...
### 3.1 aigc-patent-list (P01)

```mermaid
flowchart TB
    P[P01 aigc-patent-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[AIGC专利Form 新增/编辑表单]
    P --> Detail[AIGC专利Detail 详情弹窗]
```

### 3.2 aigc-patent-detail (P02)

```mermaid
flowchart TB
    P[P02 aigc-patent-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 aigc-patent-upload (P03)

```mermaid
flowchart TB
    P[P03 aigc-patent-upload]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /aigc-patents | aigc-patent-list页面主组件 |
| P02Page | /aigc-patents/:id | aigc-patent-detail页面主组件 |
| P03Page | /aigc-patents/form | aigc-patent-upload页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:20:38.270Z）
