...
### 实体：AIGC专利（aigc-patent）

| 字段名 | 字段标签 | 类型 | 必填 | 校验规则 | 组件映射 | 说明 |
|--------|----------|------|------|----------|----------|------|
| patent_no | 专利号 | string | 是 | 必填校验 | Input | 专利号字段 |
| algorithm_type | 算法类型 | enum | 是 | 枚举值校验 | Select | 算法类型字段 |
| code_artifact | 代码资产 | text | 否 | 最大长度 500 | TextArea | 代码资产字段 |
| ownership_type | 权属类型 | enum | 是 | 枚举值校验 | Select | 权属类型字段 |
| developer | 开发者 | string | 是 | 必填校验 | Input | 开发者字段 |
| owner | 申请人 | string | 是 | 必填校验 | Input | 申请人字段 |
| operator | 操作人 | string | 否 | - | Input | 操作人字段 |

**状态枚举**：submitted / parsing / analyzing / filed / granted
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:20:38.240Z）
