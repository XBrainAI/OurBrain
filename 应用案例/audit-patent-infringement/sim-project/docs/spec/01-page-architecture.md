...
### 3.1 infringement-case-list (P01)

```mermaid
flowchart TB
    P[P01 infringement-case-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[侵权案件Form 新增/编辑表单]
    P --> Detail[侵权案件Detail 详情弹窗]
```

### 3.2 infringement-case-detail (P02)

```mermaid
flowchart TB
    P[P02 infringement-case-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 infringement-monitor-form (P03)

```mermaid
flowchart TB
    P[P03 infringement-monitor-form]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /infringement-cases | infringement-case-list页面主组件 |
| P02Page | /infringement-cases/:id | infringement-case-detail页面主组件 |
| P03Page | /infringement-cases/form | infringement-monitor-form页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:19:16.145Z）
