...
flowchart TD
    ...
    Start --> Pool1_T1
    Pool1_Done -.->|交付侵权案件| Pool2_T1
    Pool2_Done -.->|交付侵权案件| Pool3_T1
    Pool3_Done --> End((业务完成))
```
...
|------|------|----------|----------|---------|--------|
| 品牌方IPR | 浏览侵权案件 | 登录后进入工作台 | 侵权案件列表页(infringement-case-list) | 侵权案件 | 业务数据 |
| 维权律师 | 执行业务操作 | 登录后进入工作台 | - | 侵权案件 | 业务数据 |
| 监测分析师 | 执行业务操作 | 登录后进入工作台 | - | 侵权案件 | 业务数据 |
...
```mermaid
flowchart LR
    System((本系统<br/>线上AI智能侵权监测与快速维权取证系统))
    Ext1["品牌方IPR"]
    Ext2["维权律师"]
    Ext3["监测分析师"]
    Ext1 -->|侵权案件数据| System
...
```mermaid
flowchart LR
    DS1[["侵权案件<br/>字段: case_no, platform, suspect_seller, evidence_status..."]]
    P1("新建监测<br/>(侵权案件)")
    P2("查看案件<br/>(侵权案件)")
    P3("立即存证<br/>(侵权案件)")
    P4("发起投诉<br/>(侵权案件)")
...
|------|------------|----------|----------|----------|---------|---------|
| 侵权案件 | 品牌方IPR | 品牌方IPR/维权律师/监测分析师 | 品牌方IPR/维权律师/监测分析师 | 品牌方IPR → [侵权案件] → 品牌方IPR | id | case_no, platform, suspect_seller |
...
flowchart TD
    Start((用户登录))
    P01["侵权案件列表页(infringement-case-list)<br/>/infringement-cases"]
    P02["侵权案件详情页(infringement-case-detail)<br/>/infringement-cases/:id"]
    P03["侵权案件表单页(infringement-monitor-form)<br/>/infringement-cases/form"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | 侵权案件列表页(infringement-case-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| 侵权案件列表页(infringement-case-list) | 侵权案件详情页(infringement-case-detail) | 点击"新建监测"按钮 | infringement-case | 已认证 | 路由跳转 |
| 侵权案件详情页(infringement-case-detail) | 侵权案件表单页(infringement-monitor-form) | 点击"立即存证"按钮 | infringement-case | 已认证 | 路由跳转 |
| 侵权案件表单页(infringement-monitor-form) | 侵权案件列表页(infringement-case-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T10:19:16.104Z）
