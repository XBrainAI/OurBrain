...
flowchart TD
    ...
    Start --> Pool1_T1
    Pool1_Done -.->|交付高校专利| Pool2_T1
    Pool2_Done -.->|交付高校专利| Pool3_T1
    Pool3_Done --> End((业务完成))
```
...
|------|------|----------|----------|---------|--------|
| 高校科研处 | 浏览高校专利 | 登录后进入工作台 | 高校专利列表页(college-patent-list) | 高校专利 | 业务数据 |
| 企业技术经理 | 执行业务操作 | 登录后进入工作台 | - | 高校专利 | 业务数据 |
| 转化中介 | 执行业务操作 | 登录后进入工作台 | - | 高校专利 | 业务数据 |
...
```mermaid
flowchart LR
    System((本系统<br/>高校存量专利AI盘活转化匹配平台))
    Ext1["高校科研处"]
    Ext2["企业技术经理"]
    Ext3["转化中介"]
    Ext1 -->|高校专利数据| System
...
```mermaid
flowchart LR
    DS1[["高校专利<br/>字段: patent_no, title, owner_college, industry..."]]
    P1("发起匹配<br/>(高校专利)")
    P2("查看专利<br/>(高校专利)")
    P3("进入洽谈<br/>(高校专利)")
    P4("标记转化<br/>(高校专利)")
...
|------|------------|----------|----------|----------|---------|---------|
| 高校专利 | 高校科研处 | 高校科研处/企业技术经理/转化中介 | 高校科研处/企业技术经理/转化中介 | 高校科研处 → [高校专利] → 高校科研处 | id | patent_no, title, owner_college |
...
flowchart TD
    Start((用户登录))
    P01["高校专利列表页(college-patent-list)<br/>/college-patents"]
    P02["高校专利详情页(college-patent-detail)<br/>/college-patents/:id"]
    P03["高校专利表单页(college-patent-match)<br/>/college-patents/match"]
    Start --> P01
    P01 --> P01_D{"用户操作?"}
    P01_D -->|查看详情| P02
    P01_D -->|新建| P03
...
|--------|----------|----------|------|----------|---------|
| 登录页 | 高校专利列表页(college-patent-list) | 用户登录成功 | userId | 已认证 | 路由跳转 |
| 高校专利列表页(college-patent-list) | 高校专利详情页(college-patent-detail) | 点击"发起匹配"按钮 | college-patent | 已认证 | 路由跳转 |
| 高校专利详情页(college-patent-detail) | 高校专利表单页(college-patent-match) | 点击"进入洽谈"按钮 | college-patent | 已认证 | 路由跳转 |
| 高校专利表单页(college-patent-match) | 高校专利列表页(college-patent-list) | 点击返回/面包屑 | - | 已认证 | 返回 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:57:09.095Z）
