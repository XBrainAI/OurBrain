...
### 3.1 college-patent-list (P01)

```mermaid
flowchart TB
    P[P01 college-patent-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[高校专利Form 新增/编辑表单]
    P --> Detail[高校专利Detail 详情弹窗]
```

### 3.2 college-patent-detail (P02)

```mermaid
flowchart TB
    P[P02 college-patent-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 college-patent-match (P03)

```mermaid
flowchart TB
    P[P03 college-patent-match]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /college-patents | college-patent-list页面主组件 |
| P02Page | /college-patents/:id | college-patent-detail页面主组件 |
| P03Page | /college-patents/match | college-patent-match页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:57:09.130Z）
