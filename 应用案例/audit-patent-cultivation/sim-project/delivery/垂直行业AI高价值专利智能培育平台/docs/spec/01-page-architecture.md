...
### 3.1 patent-idea-list (P01)

```mermaid
flowchart TB
    P[P01 patent-idea-list]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
    P --> Table[DataTable 数据表格]
    P --> Pagination[Pagination 分页]
    P --> Form[专利创意Form 新增/编辑表单]
    P --> Detail[专利创意Detail 详情弹窗]
```

### 3.2 patent-idea-detail (P02)

```mermaid
flowchart TB
    P[P02 patent-idea-detail]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
### 3.3 patent-idea-form (P03)

```mermaid
flowchart TB
    P[P03 patent-idea-form]
    P --> Search[SearchBar 搜索区]
    P --> Toolbar[Toolbar 操作栏]
...
| 组件 | 所属页面 | 用途 |
|------|---------|------|
| P01Page | /patent-ideas | patent-idea-list页面主组件 |
| P02Page | /patent-ideas/:id | patent-idea-detail页面主组件 |
| P03Page | /patent-ideas/form | patent-idea-form页面主组件 |
...
> 本文档由 generate-prd.mjs (v5.2.2) 基于 requirement-model.yaml 自动生成（2026-07-21T09:56:38.128Z）
